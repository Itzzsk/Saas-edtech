...
async function loadAllStudents() {
    ...
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/students/all?t=${timestamp}`, {
            method: 'GET',
            headers: {
...
async function extractDynamicData() {
    ...
    try {
        const timestamp = new Date().getTime();
        const streamsResponse = await fetch(`${API_BASE_URL}/api/students/streams?t=${timestamp}`, {
            headers: {
                'Cache-Control': 'no-cache, no-store, must-revalidate',
                'Pragma': 'no-cache'
            }
    ...
    try {
        const timestamp = new Date().getTime();
        const languagesResponse = await fetch(`${API_BASE_URL}/api/subjects/languages?t=${timestamp}`, {
            headers: {
                'Cache-Control': 'no-cache, no-store, must-revalidate',
                'Pragma': 'no-cache'
            }
    ...
    try {
        const timestamp = new Date().getTime();
        const electivesResponse = await fetch(`${API_BASE_URL}/api/students/subjects/electives?t=${timestamp}`, {
            headers: {
                'Cache-Control': 'no-cache, no-store, must-revalidate',
                'Pragma': 'no-cache'
            }
