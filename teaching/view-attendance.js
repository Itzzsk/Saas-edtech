...
async function loadStreams() {
  ...
  try {
    ...
    const url = `${API_BASE_URL}/api/streams`;
...
async function loadSemesters(stream) {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/streams/${encodeURIComponent(stream)}/semesters`, {
      headers: {
        'Cache-Control': 'no-cache',
        'Pragma': 'no-cache'
      }
...
async function loadSubjects(stream, semester) {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/subjects/${encodeURIComponent(stream)}/sem${semester}`, {
      headers: {
        'Cache-Control': 'no-cache',
        'Pragma': 'no-cache'
      }
...
async function loadRegister() {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/attendance/register/${stream}/sem${semester}/${encodeURIComponent(subject)}`);
    const data = await response.json();
...
async function loadSingleDateView() {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/attendance/date/${currentStream}/sem${currentSemester}/${encodeURIComponent(currentSubject)}/${selectedDate}`);
    const data = await response.json();
  ...
  showConfirm(message, async () => {
    try {
      ...
      const deletePromises = Array.from(selectedSessions).map(sessionId =>
        fetch(`${API_BASE_URL}/api/attendance/${sessionId}`, { 
          method: 'DELETE',
          headers: {
            'Content-Type': 'application/json'
          }
...
async function saveAttendance() {
  ...
  try {
    ...
    if (currentViewMode === 'single' && currentDateData) {
      ...
      const response = await fetch(`${API_BASE_URL}/api/attendance/bulk/${currentStream}/sem${currentSemester}/${encodeURIComponent(currentSubject)}/${currentDate}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      const updatePromises = Array.from(sessionUpdates.values()).map(update => {
        ...
        return fetch(`${API_BASE_URL}/api/attendance/session/${update.sessionId}`, {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  showConfirm(message, async () => {
    try {
      const deletePromises = currentDateData.sessions.map(session =>
        fetch(`${API_BASE_URL}/api/attendance/${session._id}`, { method: 'DELETE' })
      );
