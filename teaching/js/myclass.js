...
async function handleCreateSubject(e) {
  ...
  try {
    ...
    // Clear cached data so fresh data is loaded from server
    apiCache.data = {};
    await loadAllData();
    // Update localStorage cache for instant next load
    try { localStorage.setItem('cachedSubjects', JSON.stringify(createdSubjects)); } catch (e) { }
...
async function saveSubjectToDatabase(subjectData, retryCount = 0) {
  try {
    ...
    // Auto-retry once after 2 seconds (handles cold start / network blip)
    if (retryCount < 1) {
      console.log('🔄 Retrying in 2 seconds...');
      await new Promise(r => setTimeout(r, 2000));
      return saveSubjectToDatabase(subjectData, retryCount + 1);
    }
    throw error;
  }
}
...
async function saveQueueToDatabase() {
  try {
    ...
    try { localStorage.setItem('cachedQueue', JSON.stringify(attendanceQueue)); } catch (e) { }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Run cleanup immediately (non-blocking)
  cleanupLocalStorage();

  // Inject styles immediately (non-blocking)
  injectDropdownStyles();

  // Pre-warm the API server in background (wake up MongoDB connection)
  fetch(`${API_BASE_URL}/health`, { method: 'GET' }).catch(() => { });

  // Show cached data instantly from localStorage while real data loads
  try {
    const cachedEmail = localStorage.getItem('lastUserEmail');
    const cachedSubjects = localStorage.getItem('cachedSubjects');
    const cachedQueue = localStorage.getItem('cachedQueue');
    const cachedCompleted = localStorage.getItem('cachedCompleted');

    if (cachedSubjects) {
      try { createdSubjects = JSON.parse(cachedSubjects); } catch (e) { }
    }
    if (cachedQueue) {
      try { attendanceQueue = JSON.parse(cachedQueue); } catch (e) { }
    }
    if (cachedCompleted) {
      try { completedClasses = JSON.parse(cachedCompleted); } catch (e) { }
    }

    // Show cached UI immediately (before auth finishes)
    if (cachedEmail && (createdSubjects.length || attendanceQueue.length || completedClasses.length)) {
  ...
  try {
    ...
    try {
      localStorage.setItem('cachedSubjects', JSON.stringify(createdSubjects));
      localStorage.setItem('cachedQueue', JSON.stringify(attendanceQueue));
      localStorage.setItem('cachedCompleted', JSON.stringify(completedClasses.slice(0, 50)));
    } catch (e) { }
