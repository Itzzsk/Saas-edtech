...
async function handleTeachingLogin(email, password, name = null) {
  ...
  if (isFirstTime && name) {
    console.log('👤 Creating teaching staff profile...');
    const idToken = await user.getIdToken();
    
    const res = await fetch(`${API_BASE_URL}/api/teacher/profile`, {
      method: 'POST',
      headers: { 
    ...
    // Returning user: fetch profile
    console.log('👤 Fetching existing profile...');
    const res = await fetch(`${API_BASE_URL}/api/teacher/profile/email/${encodeURIComponent(email)}`);
    const data = await res.json();
    
    if (data.success && data.teacher) {
      console.log('✅ Profile loaded:', data.teacher.name);
    } else {
...
function checkAuthAndRedirect() {
  ...
  auth.onAuthStateChanged(user => {
    const isOnLoginPage = window.location.pathname.includes('teaching-login.html');
    
    console.log('🔍 Auth state:', { user: user?.email || 'None', isOnLoginPage });
    
    if (user && isOnLoginPage) {
      console.log('✅ Already authenticated, redirecting to myclass.html');
