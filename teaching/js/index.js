...
async function loadSubjectMetadata(stream, semester, subjectName) {
  try {
    ...
    const url = `${API_BASE_URL}/api/subjects/find?stream=${encodeURIComponent(stream)}&semester=${semester}&name=${encodeURIComponent(subjectName)}`;
...
async function loadStudentsFromDatabase(classInfo) {
  try {
    ...
    const url = `${API_BASE_URL}/api/students/${encodeURIComponent(classInfo.stream)}/sem${classInfo.semester}`;
...
async function moveQueueItemToCompleted() {
  ...
  try {
    ...
    const deleteResponse = await fetch(`${API_BASE_URL}/api/teacher/queue/${queueItemId}`, {
      method: 'DELETE',
    ...
    const completedResponse = await fetch(`${API_BASE_URL}/api/teacher/completed`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        teacherEmail: userData.userEmail,
        ...
        async () => {
          try {
            ...
            const apiUrl = currentClassInfo 
              ? `${API_BASE_URL}/api/attendance/${encodeURIComponent(currentClassInfo.stream)}/sem${currentClassInfo.semester}/${encodeURIComponent(subject)}`
