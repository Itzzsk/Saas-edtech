...
// ============================================================================
// STREAMS ROUTES (with in-memory caching for performance)
// ============================================================================

// Simple in-memory cache for streams
let streamsCache = {
  data: null,
  timestamp: 0,
...
router.get('/streams', async (req, res) => {
  try {
    ...
    // Check cache first
    const now = Date.now();
    if (streamsCache.data && (now - streamsCache.timestamp) < streamsCache.maxAge) {
      console.log('📦 Returning cached streams');
      res.set('X-Cache', 'HIT');
      return res.json(streamsCache.data);
    }
    ...
    // Update cache
    streamsCache.data = response;
    streamsCache.timestamp = now;
...
router.post('/bulk', async (req, res) => {
  try {
    ...
    // Step 3: Directly insert all valid students, allowing duplicates
    let insertedCount = 0;
    if (validStudents.length > 0) {
      const result = await req.db.collection('students').insertMany(validStudents);
      insertedCount = result.insertedCount;
      console.log(`✅ Successfully inserted ${insertedCount} students (duplicates allowed)`);
    }

    // Step 4: Build success message
    const totalProcessed = students.length;
    let message = `✅ Processed ${totalProcessed} students\n`;
    message += `✅ Added ${insertedCount} students (duplicates allowed)`;
